/**
 * @file News article detail page (dynamic route).
 * Renders a single article from /content/{locale}/news/.
 */
...
export function generateStaticParams() {
  ...
  for (const locale of locales) {
    const items = getContentItems(locale, 'news');
...
export default function NewsDetailPage({ params: { locale, slug } }: { params: { locale: string; slug: string } }) {
  setRequestLocale(locale);
  const t = useTranslations('news');
  const tcommon = useTranslations('common');
  const item = getContentItem(locale, 'news', slug);
  if (!item) notFound();

  const related = getRelatedItems(locale, 'news', slug, item.tags, 2);
    ...
    <div className="pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
        <nav className="flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>/</span>
          <Link href="/news" className="hover:text-brand-500">{t('title')}</Link>
          <span>/</span>
      ...
      <section className="py-12 bg-gradient-to-br from-brand-500 to-brand-700 text-white">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-4">
            <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-white/20 text-white">{item.category}</span>
            <span className="text-blue-200 text-sm">{item.date}</span>
          </div>
      ...
      {related.length > 0 && (
        <section className="py-16 bg-gray-50">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-8">{t('read_all')}</h2>
            <div className="grid md:grid-cols-2 gap-8">
              {related.map((r) => (
                <Link key={r.slug} href={'/news/' + r.slug} className="rounded-2xl border border-gray-200 bg-white p-6 hover:shadow-md transition-shadow">
